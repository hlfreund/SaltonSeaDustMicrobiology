...
b.kingdom_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~Kingdom, value.var="Count", fun.aggregate=sum)) ###
...
b.kingdom_m<-reshape2::melt(b.kingdom_RelAb)
...
mycomp <- list(c("PD", "BDC"), c("BDC","DP"),c("DP", "WI"),
               c("PD", "DP"), c("BDC", "WI"), c("PD", "WI"))

# sig.KO.ovview0 <- ggboxplot(
#   b.kingdom_RA_meta, x="ThreeMo_Sober", y="KO_Transformed_RelAb",
#   add = c("jitter"),
#   facet.by = "Gene.PlotID", palette=c("#15B86F", "#fb5607", "#3a86ff"),
#   nrow = 5,
#   fill = "ThreeMo_Sober",title="Ethanol Fermentation KOs - 3 Month Sobriety",size=1,add.params = list(size = 2))+
#   scale_y_continuous(name = "Transformed Relative Abundance", breaks = seq(0, 0.012, by = 0.002))+
#   rremove("x.ticks")+scale_x_discrete(labels=c("S"="Sober","NS"="Not Sober","HC" = "HC"))+
#   geom_text(data=pv.eth.info, aes(x=2, y=0.011,label=paste0("Kruskal-Wallis, p.adj = ",round(p.adj,4))),size = 6,family="Sarif") +
#   stat_compare_means(aes(group = ThreeMo_Sober),comparisons = mycomp,method="wilcox",size=6, label.y = c(0.0075, 0.0085, 0.0095)) +
#   theme(plot.title = element_text(colour = "black", size=35),
...
#
# ggsave(sig.KO.ovview0,filename = "figures/KO_RelAb/ArcsinSqRt_Transformed/EthanolFerm/ThreeMonthsSober/AUD_ThreeMonthsSober_KOs_EthFerm_ArcsSqrtTRelAb_kruskal_Sig_boxplot_a.png", width = 3000,height = 5000,units = "px",dpi = 200,create.dir = TRUE)

#### Phyla Relative Abundance ####

# use dcast to count up ASVs within each Phylum across all of the samples
b.phyla_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~Phylum, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.phyla_m<-reshape2::melt(b.phyla_RelAb)
...
p.h1<-ggplot(b.phyla_RA_meta, aes(SampleID, Phylum, fill= Count)) +geom_tile()+scale_fill_gradient2(low="blue3",mid="white",high="red",midpoint=0.45)+
  theme_classic()+theme(axis.title.x = element_text(size=13,vjust=-0.5),axis.title.y = element_text(size=13),axis.text = element_text(size=11),axis.text.x = element_text(angle=40, vjust=.93, hjust=1.01),legend.title.align=0.5, legend.title = element_text(size=13),legend.text = element_text(size=11),plot.title = element_text(size=15)) +
  labs(x="Sample ID", y="Microbial Phyla", title="",fill="Relative Abundance")+scale_x_discrete(expand = c(0,0)) +
  facet_grid(. ~ SampDate, scales = "free")

ggsave(p.h1,filename = "figures/RelativeAbundance/Phylum/SSD_16S_Phyla.RA_SampDate_heatmap.png", width=16, height=10, dpi=600,create.dir = TRUE)

p.h2<-ggplot(b.phyla_RA_meta, aes(SampleID, Phylum, fill= Count)) +geom_tile()+scale_fill_gradient2(low="blue3",mid="white",high="red",midpoint=0.45)+
  theme_classic()+theme(axis.title.x = element_text(size=20,vjust=-0.5),axis.title.y = element_text(size=20),axis.text = element_text(size=16),
                        axis.text.x = element_text(angle=40, vjust=.93, hjust=1.01),strip.text.x = element_text(size = 16,face="bold"),
                        legend.title.align=0.5, legend.title = element_text(size=17),legend.text = element_text(size=15),) +
  labs(x="Sample ID", y="Microbial Phyla", title="",fill="Relative Abundance")+scale_x_discrete(expand = c(0,0)) +
  facet_grid(. ~ Site, scales = "free")

ggsave(p.h2,filename = "figures/RelativeAbundance/Phylum/SSD_16S_Phyla.RA_Site_heatmap.png", width=18, height=15, dpi=600,create.dir = TRUE)
...
# use dcast to count up ASVs within each Phylum across all of the samples
b.gram_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~GramID, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.gram_m<-reshape2::melt(b.gram_RelAb)
...
b.class_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~Class, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.class_m<-reshape2::melt(b.class_RelAb)
...
b.ord_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~Order, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.ord_m<-reshape2::melt(b.ord_RelAb)
...
b.fam_counts <- as.data.frame(reshape2::dcast(b.dust.all, SampleID~Family, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.fam_m<-reshape2::melt(b.fam_RelAb)
...
# create directory for storing maaslin3 results
mainDir="/Volumes/LF_SSD2/Aronson_Lab_Data/Salton_Sea/SaltonSeaDust/figures/RelativeAbundance"
subDir="/Maaslin_Results/"
dir.create(file.path(mainDir, subDir), showWarnings = TRUE)

# make sure b.genus_RelAb and metadata are in same order rownames
dust_meta=dust_meta[rownames(b.fam_RelAb),] ## will drop rows that are not shared by both dataframes!
rownames(dust_meta) %in% rownames(b.fam_RelAb)
which(colnames(b.fam_RelAb)=="SampleID")
...
# first maaslin3 run --> bdc is our reference for comparison
fit_data.fam  = maaslin3(input_data = b.fam_RelAb[,-504], # < input would be relative abundances
                           input_metadata = dust_meta,
                           formula = '~ Site',
                           transform = "NONE",      # < the addition of the AST
                           normalization  = "NONE",
                           output         = "figures/RelativeAbundance/Maaslin_Results/Fam_Maaslin",
                           min_prevalence = 0.1,
                           #fixed_effects  = c("Site"),
                           reference = c('Site,BDC'),
                           warn_prevalence = FALSE,
                           standardize = TRUE,
                           save_models = TRUE)

# # by Family + depth
# bac.fam.dep <- as.data.frame(reshape2::dcast(b.dust.all,Depth_m~Family, value.var="Count", fun.aggregate=sum)) ###
...
#
# #melt down relativized data to merge with dust_meta
# b.fam.dep_m<-reshape2::melt(b.RA_fam.dep, by="Depth_m")
#
# head(b.fam.dep_m)
...
# bac.fam.date <- as.data.frame(reshape2::dcast(b.dust.all,SampDate~Family, value.var="Count", fun.aggregate=sum)) ###
...
#
# #melt down relativized data to merge with dust_meta
# b.fam.date_m<-reshape2::melt(b.RA_fam.date, by="SampDate")
#
# head(b.fam.date_m)
...
b.genus_counts <- as.data.frame(reshape2::dcast(b.dust.all.g, SampleID~Genus, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.genus_m<-reshape2::melt(b.genus_RelAb)
...
spec.gen.plt1<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Adhaeribacter",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  scale_y_continuous(name = "Relative Abundance")+
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.1) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        text = element_text(colour = "black", size=22),
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt1,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Adhaeribacter_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt2<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Segetibacter",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.13) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt2,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Segetibacter_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt3<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Bombilactobacillus",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  theme(plot.title = element_text(colour = "black", size=35),
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt3,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Bombilactobacillus_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt4<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Flavisolibacter",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.12) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt4,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Flavisolibacter_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt5<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Massilia",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 1.23) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt5,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Massilia_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt6<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Edaphobaculum",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.011) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt6,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Edaphobaculum_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt7<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Cystobacter",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.0065) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt7,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Cystobacter_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)

spec.gen.plt8<-ggboxplot(
  b.genus_RA_meta[b.genus_RA_meta$Genus=="Symplocastrum CPER KK1",], x="Site", y="Count",
  add = c("jitter"),
  facet.by = "Genus",
  nrow = 5,
  fill = "Site",title="",size=1,add.params = list(size = 2))+
  ...
  scale_color_manual(name ="Site",values=unique(b.genus_RA_meta$Site_Color[order(b.genus_RA_meta$Site)])) +
  rremove("x.ticks")+
  stat_compare_means(aes(group = Site),method="kruskal",size=6,label.y = 0.027) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6) +
  theme(plot.title = element_text(colour = "black", size=35),
        axis.title.x = element_blank(),
        legend.position = "none",
        ...
        strip.text.x = element_text(size = 19, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))
ggsave(spec.gen.plt8,filename = "figures/Revisions/RelativeAbundance/Genus/SSD_Symplocastrum.CPER.KK1_boxplot.png", width = 3000,height = 3000,units = "px",dpi = 200,create.dir = TRUE)
...
# make sure b.genus_RelAb and metadata are in same order rownames
dust_meta=dust_meta[rownames(b.genus_RelAb),] ## will drop rows that are not shared by both dataframes!
rownames(dust_meta) %in% rownames(b.genus_RelAb)

## default method for model is linear regression
## if we add fixed & random effects, uses lme4 linear mixed-effect model: https://cran.r-project.org/web/packages/lme4/index.html
## more here: https://github.com/biobakery/biobakery/wiki/maaslin2#31-maaslin-2-input
# first maaslin3 run --> HC is our reference for comparison
fit_data.genus  = maaslin3(input_data = b.genus_RelAb[,-1347], # < input would be relative abundances
                                   input_metadata = dust_meta,
                                   formula = '~ Site',
                                   transform = "NONE",      # < the addition of the AST
                                   normalization  = "NONE",
                                   output         = "figures/RelativeAbundance/Maaslin_Results/Genus_Maaslin",
                                   min_prevalence = 0.1,
                                   #fixed_effects  = c("Site"),
                                   reference = c('Site,BDC'),
                                   warn_prevalence = FALSE,
...
# r
#ggsave(genbxplt1,filename = "figures/Revisions/SSD_Genera_RelAb_kruskal_Sig_boxplot_a.png", width = 3000,height = 5000,units = "px",dpi = 200,create.dir = TRUE)
...
b.gen.spec_counts <- as.data.frame(reshape2::dcast(b.dust.all.g, SampleID~Genus+Species, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.gen.spec_m<-reshape2::melt(b.gen.spec_RelAb)
...
b.k.genus_counts <- as.data.frame(reshape2::dcast(b.dust.all.g, SampleID~Genus+Species+Kingdom, value.var="Count", fun.aggregate=sum)) ###
...
# melt down relativized data to merge with dust_meta
b.k.genus_m<-reshape2::melt(b.k.genus_RelAb)
...
b.gs.df <- as.data.frame(reshape2::dcast(b.gen.spec_m, SampleID~Genus_species, value.var="Count", fun.aggregate=sum)) ###
...
rownames(b.gs.mat)
rownames(b.gs.mat)<-gsub("unknown","sp.",rownames(b.gs.mat))
...
ggsave(core.p,filename = "figures/Revisions/RelativeAbundance/CoreMicrobiome/SSD_16S_CoreMicrobiomeGenera_heatmap.png", width=20, height=15, dpi=600,create.dir = TRUE)
...
gencol = reshape2::melt(c(Massilia="cyan3",Sphingomonas="purple1",Planomicrobium="greenyellow", Hymenobacter="darkgoldenrod1", Planococcus="tomato1",
...
core.genus.meta<-merge(b.genus_RA_meta, gencol, by="Genus")
...
core.gen.barplot<-ggplot(core.genus.meta, aes(x=SampleID, y=Count, fill=Genus))+geom_bar(stat="identity",colour="black")+scale_x_discrete()+theme_classic()+
  labs(x="SampleID", y="Relative Abundance", subtitle="Core Microbiome Taxa Only",fill="Genus")+
  theme(axis.title.x = element_text(size=20),axis.title.y = element_text(size=20),axis.text = element_text(size=16),
        axis.text.x = element_text(hjust=1,angle=45),legend.title.align=0.5, legend.title = element_text(size=20),
        legend.text = element_text(size=16),strip.text.x = element_text(size = 25, face = "bold"),
        strip.background=element_rect(fill="ghostwhite"))+
  guides(fill=guide_legend(ncol=1))+facet_wrap(vars(Site), scales = "free") +
  scale_fill_manual(name ="Genus",values=unique(core.genus.meta$Genus_Color[order(core.genus.meta$Genus)]))

ggsave(core.gen.barplot,filename = "figures/RelativeAbundance/CoreMicrobiome/SSD_16S_CoreMicrobiomeGenera_barplot_v2.png", width=25, height=25, dpi=600,create.dir = TRUE)
...
# b.g.site <- as.data.frame(reshape2::dcast(b.dust.all.g, Site+SampDate~Genus+Species, value.var="Count", fun.aggregate=sum)) ###
...
#
# # melt down relativized data to merge with dust_meta
# b.g.site_RA.m<-reshape2::melt(b.g.site_RA)
#
# head(b.g.site_RA.m)
...
#
# gencol = reshape2::melt(c(Bacillus="darkgreen",Blastococcus="orange",Geodermatophilus="green3", Halomonas="darkslategray4", Marinobacter="blue3", Other="black", Paracoccus="firebrick1", Roseovarius="deeppink3", Salinicoccus="cornflowerblue", Sphingomonas="purple", Spiroplasma="deepskyblue1", Truepera="darkgoldenrod2"))
# head(gencol)
