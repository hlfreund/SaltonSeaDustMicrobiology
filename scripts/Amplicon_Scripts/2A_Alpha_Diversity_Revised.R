...
#### Prep Un-Scaled ASV Table ####
keep.reps<-c("\\.A","\\.C")
bac.ASV_table_og2<-bac.ASV_table_og
bac.ASV_table_og2<-bac.ASV_table_og2[grep(paste(keep.reps,collapse="|"),bac.ASV_table_og2$SampleID),] # drop technical reps of samples
bac.ASV_table_og2$SampleID<-gsub("(.*\\..*)\\..*","\\1", bac.ASV_table_og2$SampleID)
bac.ASV_table_og2$SampleID[grep("DP.D.9.8.21", bac.ASV_table_og2$SampleID)] <- "DP.D.9.18.21"

# sanity check
bac.ASV_table_og2$SampleID[bac.ASV_table_og2$SampleID %in% bac.ASV_table$SampleID]

# subset
bac.ASV_table_og2<-bac.ASV_table_og2[bac.ASV_table_og2$SampleID %in% bac.ASV_table$SampleID,]
rownames(bac.ASV_table_og2)<-bac.ASV_table_og2$SampleID

rowSums(bac.ASV_table_og2[,-1])
# drop two samples with lowest total reads, WI.D.10.10.20.A and PD.D.10.8.20.A
min(rowSums(bac.ASV_table_og2[,-1]))

bac.ASV_table_og2<-subset(bac.ASV_table_og2,!bac.ASV_table_og2$SampleID %in% c("WI.D.10.10.20","PD.D.10.8.20"))
...
par(mar=c(1,1,1,1))
sc2<-specaccum(bac.ASV_table_og2[,-1],"random")
...
rowSums(bac.ASV_table_og2[,-1]) # total # ASVs per sample, excluding SampleID from calculation
sort(colSums(bac.ASV_table_og2[,-1])) # counts per ASV
sort(rowSums(bac.ASV_table_og2[,-1])) # ASVs per sample

# Create Rarefaction curve
# png('figures/AlphaDiversity/SSD_16S_rarecurve.png')
par(mar=c(1,1,1,1))
rarecurve(as.matrix(bac.ASV_table_og2[,-1]),col=meta.all.scaled$SampDate_Color, step=1000, label=F,ylab="ASVs")
...
# bac.ASV.melt<-melt(bac.ASV_table_og2,by="SampleID")
...
# sings<-data.frame(Singletons=rowSums(bac.ASV_table_og2[,-1]==1),SampleID=bac.ASV_table_og2$SampleID) # how many ASVs have a count of only 1
# # no singletons?
#
# # find total counts per sample
# totseq<-data.frame(TotalSeqs=rowSums(bac.ASV_table_og2[,-1]),SampleID=bac.ASV_table_og2$SampleID) # total # of counts per sample
#
...
min.rar<-min(rowSums(bac.ASV_table_og2[,-1])) ## seeing min sum of OTUs so we can see what min is for rarefaction
min.rar

bac.ASV.rar1<-rarefy(bac.ASV_table_og2[,-1],min.rar) ## be cognizant of min for rarefaction
bac.ASV.rar1 # rarefy gives the expected species richness in random subsamples of size sample from the community

bac.ASV.rar<-rrarefy(bac.ASV_table_og2[,-1],min.rar)
...
ave.sdiv <- data.frame(AveShanDiv=rowMeans(data.frame(lapply(as.list(1:100), function(x) sdiv.rarefy(bac.ASV_table_og2[,-1], min.rar)))))
# sdiv.rarefy function --> Rrarefy ASV warnings() table with given min, calculate Shannon diversity
# lapply(as.list(1:100), function(x) sdiv.rarefy(bac.ASV_table_og2[,-1], min.rar))) --> creates list by running sdiv.rarefy() function 100 times & storing results in list each time
...
bac.ASV.probs<-drarefy(bac.ASV_table_og2[,-1],min.rar)
...
# vv contains average Shannon Diversity calculations after rarefaction and Shan div calculations 100 times
ave.spec.rich <- data.frame(AveSpecRich=rowMeans(data.frame(lapply(as.list(1:100), function(x) spech.rich.rarefy(bac.ASV_table_og2[,-1], min.rar)))))
# spec.rich.rarefy function --> Rrarefy ASV warnings() table with given min, calculate Shannon diversity
# lapply(as.list(1:100), function(x) sdiv.rarefy(bac.ASV_table_og2[,-1], min.rar))) --> creates list by running spec.rich.rarefy() function 100 times & storing results in list each time
# data.frame(lapply(...)) --> saves lapply() output as data frame, not list
...
specnumber(bac.ASV_table_og2[,-1])
...
# bac.ASV_table_og2[1:5,1:5]
#
# #bac.ASV_matrix<-as.matrix(bac.ASV_table_og2[,!names(bac.ASV_table_og2) %in% c("SampleID")]) # convert count table into matrix & exclude column called SampleID
# bac.ASV_matrix2<-t(as.matrix(bac.ASV_table_og2[,-1])) # transpose matrix so ASVs are rows, samples are columns
...
bac.ASV_table_og2$SampleID = factor(bac.ASV_table_og2$SampleID, levels=unique(bac.ASV_table_og2$SampleID[order(rowSums(bac.ASV_table_og2[,-1]))]), ordered=TRUE)

raw.tot.counts<-ggplot(data=bac.ASV_table_og2, aes(x=bac.ASV_table_og2$SampleID, y=rowSums(bac.ASV_table_og2[,-1]))) +
...
# mycomp <- list(c("PD", "BDC"), c("PD", "DP"), c("PD", "WI"),
#                c("BDC","DP"), c("BDC", "WI"), c("DP", "WI"))
...
bac.a.div.rar<-ggplot(bac.div.metadat.rar, aes(x=Site, y=AveShanDiv)) +geom_jitter(aes(color=Site), size=3, width=0.15, height=0) +
  ...
  labs(title = "Dust Bacterial Shannon Diversity by Site", subtitle="Using Rarefied Counts", x="Site", y="Shannon Diversity", color="Depth (m)")+
  ...
  stat_compare_means(aes(group = Site),label.y = 2550) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox")
...
bac.a.div.rarB<-ggplot(bac.div.metadat.rar, aes(x=Site, y=AveShanDiv)) +geom_jitter(aes(color=SampDate,shape=Site), size=5, width=0.15, height=0) +
  ...
  labs(x="Site", y="Shannon Diversity", color="Collection Date")+
  theme(axis.title.x = element_text(size=20),axis.title.y = element_text(size=20),axis.text = element_text(size=15),axis.text.x = element_text(vjust=1,size=15),legend.title.align=0.5, legend.title = element_text(size=16),legend.text = element_text(size=15))+
  ...
  stat_compare_means(aes(group = Site),label.y = 2600, size=6) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6)

ggsave(bac.a.div.rarB,filename = "figures/Revisions/AlphaDiversity/RarefiedCounts/SSD_16S_rarefied_alpha_diversity_sampledate_site_boxplot.png", width=15, height=10, dpi=600,create.dir = TRUE)
...
bac.a.sr.rarB<-ggplot(bac.div.metadat.rar, aes(x=Site, y=AveSpecRich)) +geom_jitter(aes(color=SampDate,shape=Site), size=5, width=0.15, height=0) +
  scale_color_manual(name ="Collection Date",values=unique(bac.div.metadat.rar$SampDate_Color[order(bac.div.metadat.rar$SampDate)]),labels=c("July 2020", "August 2020", "October 2020","November 2020", "July 2021", "August 2021", "September 2021", "December 2021")) +
  geom_boxplot(fill=NA, outlier.color=NA)+theme_bw()+theme_classic()+
  labs(y="Species Richness", color="Collection Date")+
  theme(axis.title.x = element_text(size=20),axis.title.y = element_text(size=20),axis.text = element_text(size=15),axis.text.x = element_text(vjust=1,size=15),legend.title.align=0.5, legend.title = element_text(size=16),legend.text = element_text(size=15))+
  scale_shape_manual(values = c(0,1,16,15),labels=unique(bac.div.metadat.rar$Site[order(bac.div.metadat.rar$Site)])) + scale_x_discrete(labels=unique(bac.div.metadat.rar$Site[order(bac.div.metadat.rar$Site)]))+
  stat_compare_means(aes(group = Site),label.y = 3200, size=6) +
  stat_compare_means(aes(group = Site),comparisons = mycomp,method="wilcox",size=6)

ggsave(bac.a.sr.rarB,filename = "figures/Revisions/SpeciesRichness/RarefiedCounts/SSD_16S_rarefied_species_richness_sampledate_site_boxplot.png", width=15, height=10, dpi=600,create.dir = TRUE)
